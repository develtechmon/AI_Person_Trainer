...
class utilities:
    ...
    def display_rep_count(self, img, count, total_reps):
        cv2.rectangle(img, (0, 0), (110, 180), (255, 255, 255), cv2.FILLED)
        cv2.putText(
            img,
    ...
    def draw_performance_bar(self, img, per, bar, color, count):
        ...
        cv2.rectangle(img, (600, 100), (610, 450), color, 3)
        cv2.rectangle(img, (600, int(bar)), (610, 450), color, cv2.FILLED)
        
        cv2.putText(
            img, f"{int(per)}%", (550, 80), cv2.FONT_HERSHEY_PLAIN, 3, color, 4
        )
...
class simulate_target_exercies:
    def __init__(self, difficulty_level=1, reps=4):
        self.reps = reps
        self.difficulty_level = difficulty_level
    ...
    def bicep_curls(self):
        ...
        while count <= total_reps:
            ...
            if len(landmark_list) != 0:
                ...
                bar = np.interp(right_arm_angle, (50, 160), (450, 100))
